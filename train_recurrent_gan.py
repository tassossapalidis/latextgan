...
from recurrent_gan import Generator, Discriminator
...
generator = Generator(2, noise_dim, units*2, BATCH_SIZE, 30)
...
def train_step_disc(real_data_batch):
    ...
    with tf.GradientTape() as tape:
        ...
        generator_output = tf.convert_to_tensor(np.empty(shape = (BATCH_SIZE, 0, units*2)), 
                                                dtype=tf.float32)
        #prev_word = tf.tf.Tensor([])
        for i in range(max_sentence_length):
            z = tf.random.normal((BATCH_SIZE, units*2))
            ## pad to max_sequence_length using zeros (mask tokens)
            inp = tf.pad(generator_output, [[0, 0], [0, max_sentence_length-i], [0,0]])
            generated_word = generator(z, inp)
            generator_output = tf.concat((generator_output, tf.expand_dims(generated_word, 1)), axis = 1)
        ...
        fake_predictions = discriminator(generator_output)
        # TODO: change this to real predictions
        disc_loss = discriminator_loss(fake_predictions, fake_predictions)
